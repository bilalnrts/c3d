...
void    ft_calculate_step(t_map *map)
{
    ...
    {
        map->step_x = -1;
        map->side_dist_x = (map->player->x - map->player->map_x) * map->delta_dist_x;
    }
    else
    {
        map->step_x = 1;
        map->side_dist_x = (map->player->map_x + 1.0 - map->player->x) * map->delta_dist_x;
    }
    if (map->player->ray_dir_y < 0)
    {
        map->step_y = -1;
        map->side_dist_y = (map->player->y - map->player->map_y) * map->delta_dist_y;
    }
    else
    {
        map->step_y = 1;
        map->side_dist_y = (map->player->map_y + 1.0 - map->player->y) * map->delta_dist_y;
    }
}
